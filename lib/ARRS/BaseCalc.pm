...
#  return '-'.$self->to_base(-1*$num) if $num<0; # Handle negative numbers
  ...
  my $result = '';
  while (!$num->is_zero && !$num->is_negative && $num->is_positive()) {
    my $tempnum = $num->copy();
    $tempnum->bmod($dignum);
    substr($result,0,0) = $self->{digits}[ $tempnum ];
    $num->bdiv($dignum);
  }
